...
module FIFO_tb();
    ...
    parameter DATASIZE = 8; // Data bus size
    parameter ADDRSIZE=4; // Address bus size
    parameter DEPTH = 1 << ADDRSIZE; // Depth of the FIFO memory

    reg [DATASIZE-1:0] wdata;   // Input data
    wire [DATASIZE-1:0] rdata;  // Output data
    wire wfull, rempty;      // Write full and read empty signals
    reg winc, rinc, wclk, rclk, wrst_n, rrst_n; // Write and read signals

    fifo_top #(DATASIZE, ADDRSIZE) fifo (
        .rdata(rdata), 
        .wdata(wdata),
        .wfull(wfull),
        .rempty(rempty),
